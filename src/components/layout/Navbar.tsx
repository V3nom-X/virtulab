    ...
    <header className="fixed top-0 left-0 right-0 z-50 h-16 glass-panel border-b">
      <div className="flex items-center justify-between h-full px-4 md:px-6 max-w-[1800px] mx-auto">
        ...
        <div className="flex items-center gap-2">
          <div className={`relative transition-all duration-300 ${isSearchOpen ? "w-64" : "w-auto"}`}>
            {isSearchOpen ? (
              <div className="relative">
                ...
                <Input type="search" placeholder="Search experiments..." className="pl-9 pr-9 h-9 bg-muted/50" autoFocus onBlur={() => setIsSearchOpen(false)} />
              ...
              <Button variant="ghost" size="icon" onClick={() => setIsSearchOpen(true)} className="text-muted-foreground">
          ...
          <Link to="/settings">
            <Button variant="ghost" size="icon" className="hidden sm:flex text-muted-foreground"><Settings className="w-5 h-5" /></Button>
          </Link>
          <Link to="/help">
            <Button variant="ghost" size="icon" className="hidden sm:flex text-muted-foreground"><HelpCircle className="w-5 h-5" /></Button>
          </Link>
          ...
          {user ? (
            ...
            <Link to="/auth">
              <Button size="sm" className="gap-1"><LogIn className="w-4 h-4" />Sign In</Button>
            </Link>
          )}

          <Button variant="ghost" size="icon" className="lg:hidden text-muted-foreground" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
