...
export function detectInitialTier(): DeviceTier {
  ...
  const ram = typeof nav.deviceMemory === "number" ? nav.deviceMemory : undefined;
  ...
  if ((ram !== undefined && ram <= 2) || cores <= 2) return "low";
  if ((ram !== undefined && ram <= 4) || cores <= 4 || (smallScreen && dpr >= 2)) return "medium";
  return "high";
}
