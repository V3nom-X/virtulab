...
import { SeparationSimulation } from "@/components/separations/SeparationSimulation";
import { SeparationAnalysis } from "@/components/separations/SeparationAnalysis";
...
const SeparationExperiment = () => {
  ...
  const experiment = separationExperiments.find((e) => e.id === experimentId);
    ...
    return (
      <Layout>
        <div className="container py-20 text-center">
          ...
          <Link to="/separation-of-mixtures">
  ...
  const nextExperiment = separationExperiments[separationExperiments.findIndex((e) => e.id === experimentId) + 1];

  return (
    <Layout>
      <div className="min-h-screen">
        {/* Header */}
        <section className="py-6 border-b" style={{ background: "linear-gradient(135deg, hsl(120 100% 15% / 0.08), transparent)" }}>
          <div className="container">
            <Link to="/separation-of-mixtures" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground mb-3">
          ...
          <div className="container">
            <Tabs defaultValue="overview">
              <TabsList className="mb-6 flex-wrap h-auto gap-1">
                ...
                <TabsTrigger value="analysis" className="gap-1"><BarChart2 className="w-4 h-4" />Analysis</TabsTrigger>
                  ...
                  <div className="space-y-3">
                    {experiment.howItWorks.map((step) => (
                      <div key={step.step} className="flex gap-3 p-3 bg-muted/50 rounded-lg">
                        <div className="w-8 h-8 rounded-full bg-[hsl(120,100%,20%)] text-white flex items-center justify-center text-sm font-bold shrink-0">
                          {step.step}
                        </div>
                        <div>
              ...
              <TabsContent value="applications">
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {experiment.applications.map((app, i) => (
                    <div key={i} className="p-4 bg-card border rounded-lg hover-lift">
              ...
              <TabsContent value="simulation">
                <SeparationSimulation ref={simRef} experimentId={experiment.id} />
              </TabsContent>

              {/* Analysis */}
              <TabsContent value="analysis">
                <SeparationAnalysis experimentId={experiment.id} />
              </TabsContent>
                  ...
                  <ul className="space-y-2">
                    {experiment.summary.map((point, i) => (
                      <li key={i} className="flex items-start gap-2">
                        <CheckCircle2 className="w-5 h-5 text-[hsl(120,100%,20%)] shrink-0 mt-0.5" />
                        <span>{point}</span>
                      </li>
                    ))}
                  </ul>
                ...
                <div className="border rounded-lg p-6">
                            ...
                            return (
                              <button
                                ...
                                className={`text-left p-3 rounded-lg border text-sm transition-colors ${
                                  ...
                                  wrong ? "bg-red-100 dark:bg-red-900/30 border-red-500" :
                  ...
                  {!quizSubmitted ? (
                    ...
                    <div className="mt-4 space-y-3">
                      ...
                      {quizScore === experiment.quizQuestions.length && (
                        <Badge className="bg-gold text-gold-foreground">🏆 Perfect Score!</Badge>
                      )}
                      <div className="flex gap-2">
                        ...
                        {nextExperiment && (
                          <Link to={`/separation-of-mixtures/${nextExperiment.id}`}>
                            <Button className="bg-[hsl(120,100%,20%)] hover:bg-[hsl(120,100%,15%)] text-white">
...
export default SeparationExperiment;
