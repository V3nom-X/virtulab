...
const AcidBaseExperiment = () => {
  ...
  const experiment = acidBaseExperiments.find((e) => e.id === experimentId);
  const [quizAnswers, setQuizAnswers] = useState<Record<number, number>>({});
  const [quizSubmitted, setQuizSubmitted] = useState(false);
  const { markComplete } = useAcidBaseProgress();
    ...
    return (
      <Layout>
        <div className="container py-20 text-center">
          ...
          <Link to="/acids-bases-indicators">
  ...
  const nextExperiment = acidBaseExperiments[acidBaseExperiments.findIndex((e) => e.id === experimentId) + 1];

  return (
    <Layout>
      <div className="min-h-screen">
        {/* Header */}
        <section className="py-6 border-b" style={{ background: "linear-gradient(135deg, hsl(280 80% 20% / 0.08), transparent)" }}>
          <div className="container">
            <Link to="/acids-bases-indicators" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground mb-3">
                  ...
                  {!quizSubmitted ? (
                    ...
                    <div className="mt-4 space-y-3">
                      ...
                      <div className="flex gap-2">
                        ...
                        {nextExperiment && (
                          <Link to={`/acids-bases-indicators/${nextExperiment.id}`}>
                            <Button>
