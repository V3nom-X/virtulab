...
import { SourcesOfElectricitySimulation } from "@/components/forceenergy/SourcesOfElectricitySimulation";
import { FlowOfCurrentSimulation } from "@/components/forceenergy/FlowOfCurrentSimulation";
import { UsesOfElectricitySimulation } from "@/components/forceenergy/UsesOfElectricitySimulation";
import { forceEnergyExperiments } from "@/data/forceEnergyData";
import { ArrowLeft, CheckCircle2, Globe, BookOpen, FlaskConical, Trophy } from "lucide-react";
import { useState } from "react";
import { useForceEnergyProgress } from "@/hooks/useForceEnergyProgress";

const getSimulationComponent = (experimentId: string) => {
  switch (experimentId) {
    case "sources-of-electricity": return <SourcesOfElectricitySimulation />;
    case "flow-of-electric-current": return <FlowOfCurrentSimulation />;
    case "uses-of-electricity": return <UsesOfElectricitySimulation />;
    default: return <SourcesOfElectricitySimulation />;
  }
};

const ForceEnergyExperiment = () => {
  const { experimentId } = useParams<{ experimentId: string }>();
  const experiment = forceEnergyExperiments.find((e) => e.id === experimentId);
  const [quizAnswers, setQuizAnswers] = useState<Record<number, number>>({});
  const [quizSubmitted, setQuizSubmitted] = useState(false);
  const { markComplete } = useForceEnergyProgress();
    ...
    return (
      <Layout>
        <div className="container py-20 text-center">
          ...
          <Link to="/force-and-energy"><Button>Back to module</Button></Link>
        </div>
      </Layout>
    );
  }
  ...
  const nextExperiment = forceEnergyExperiments[forceEnergyExperiments.findIndex((e) => e.id === experimentId) + 1];

  return (
    <Layout>
      <div className="min-h-screen">
        <section className="py-6 border-b" style={{ background: "linear-gradient(135deg, hsl(45 80% 20% / 0.08), transparent)" }}>
          <div className="container">
            <Link to="/force-and-energy" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground mb-3">
                  ...
                  {!quizSubmitted ? (
                    ...
                    <div className="mt-4 space-y-3">
                      ...
                      <div className="flex gap-2">
                        ...
                        {nextExperiment && (
                          <Link to={`/force-and-energy/${nextExperiment.id}`}>
...
export default ForceEnergyExperiment;
