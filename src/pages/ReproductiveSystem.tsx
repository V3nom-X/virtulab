...
const ReproductiveSystem = () => {
  const { completed, total } = useReproductiveProgress();
  const progress = (completed.length / total) * 100;

  return (
    <Layout>
      <div className="min-h-screen">
        <section className="py-12 border-b" style={{ background: "linear-gradient(135deg, hsl(330 50% 20% / 0.1), hsl(280 40% 20% / 0.05))" }}>
          <div className="container">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h1 className="text-3xl md:text-4xl font-bold">Human Reproductive System</h1>
                <p className="text-muted-foreground">Explore male and female reproductive anatomy and function</p>
              </div>
            </div>
        ...
        <section className="py-8">
          <div className="container">
            <div className="grid md:grid-cols-2 gap-6">
              {reproductiveExperiments.map((exp, i) => {
                const isComplete = completed.includes(exp.id);
                return (
                  <Link key={exp.id} to={`/reproductive-system/${exp.id}`} className="group block animate-fade-in" style={{ animationDelay: `${i * 100}ms` }}>
                    <div className="bg-card rounded-xl border overflow-hidden hover-lift h-full">
                      <div className="h-32 flex items-center justify-center" style={{ background: "linear-gradient(135deg, hsl(330 50% 25%), hsl(280 40% 20%))" }}>
