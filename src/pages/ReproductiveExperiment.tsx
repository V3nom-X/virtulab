...
const ReproductiveExperiment = () => {
  const { experimentId } = useParams<{ experimentId: string }>();
  const experiment = reproductiveExperiments.find((e) => e.id === experimentId);
  const [quizAnswers, setQuizAnswers] = useState<Record<number, number>>({});
  const [quizSubmitted, setQuizSubmitted] = useState(false);
  const { markComplete } = useReproductiveProgress();
    ...
    return (
      <Layout>
        <div className="container py-20 text-center">
          ...
          <Link to="/reproductive-system"><Button>Back to module</Button></Link>
        </div>
      </Layout>
    );
  }
  ...
  const systemType = experimentId === "male-reproductive-system" ? "male" : "female";
  const nextExperiment = reproductiveExperiments[reproductiveExperiments.findIndex((e) => e.id === experimentId) + 1];

  return (
    <Layout>
      <div className="min-h-screen">
        <section className="py-6 border-b" style={{ background: "linear-gradient(135deg, hsl(330 50% 20% / 0.08), transparent)" }}>
          <div className="container">
            <Link to="/reproductive-system" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground mb-3">
                  ...
                  {!quizSubmitted ? (
                    ...
                    <div className="mt-4 space-y-3">
                      ...
                      <div className="flex gap-2">
                        ...
                        {nextExperiment && (
                          <Link to={`/reproductive-system/${nextExperiment.id}`}>
...
export default ReproductiveExperiment;
